...
install_backend() {
    ...
    # uv resolves from the project root pyproject.toml; run from parent dir
    cd ..
    ...
    if uv run python -c "import uvicorn; import fastapi" >/dev/null 2>&1; then
        ...
        if uv run python -c "import uvicorn; import fastapi" >/dev/null 2>&1; then
            ...
            print_error "Try running: uv sync --reinstall"
            exit 1
        fi
    fi
...
start_services() {
    ...
    # Run from the project root (parent of app/) to ensure proper Python imports
    cd ..
